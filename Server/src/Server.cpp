    ...
    if (selector.wait()) {
      ...
      if (selector.isReady(*listener_)) {
        ...
        if (listener_->accept(*client) == sf::Socket::Status::Done) {
          std::string msg = "Client connected from " +
        ...
        // There is no pending connection. Lets check each current client for
        // messages
        for (auto it = clients.begin(); it != clients.end();) {
          ...
          if (selector.isReady(client)) {
            // We've received a message. Should be in sf::Packet form, so just
            // call receive()
            sf::Packet receivedData;
            sf::Socket::Status status = client.receive(receivedData);

            if (status == sf::Socket::Done) {
              // Copy the receivedData message to a string, and print that
              // string to the log
              std::string newMsg;
              receivedData >> newMsg;
              ...
              writeToLog("New message received from from " +
                         client.getRemoteAddress().toString() + ":" +
                         std::to_string(client.getRemotePort()) + ": " +
                         newMsg);
              ...
              std::string msg = "Client disconnected from " +
                                client.getRemoteAddress().toString() + ":" +
                                std::to_string(client.getRemotePort()) + "\n";
              writeToLog(msg);

              // Remove the client from the selector, and from the vector of
              // clients
