...
class Client {
 public:
   ...
   * @return A true or false on whether to continue prompting (based on whether
   * the user opted to disconnect)
   */
  bool promptMessage();
   ...
   *
   * @param The message to send to the server, obtained through the console via
   * promptMessage
   */
 ...
 private:
  ...
  std::unique_ptr<sf::TcpSocket> socket_ =
      std::make_unique<sf::TcpSocket>();  // A pointer to the socket
};
