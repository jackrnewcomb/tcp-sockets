...
bool sanitizeInput(const std::vector<std::string>& userInput) {
  ...
  std::string invalidMsg =
      "Invalid command line argument detected : " + userInput[1] +
      "\nPlease check your values and press any key to end the program!";

  // Try casting the port input to an int. If it fails, the user inputted an
  // invalid string
  int port;
  try {
  ...
  // If port is not within the valid port range. Also implicitly handles error
  // catching for negative ports
...
/**
 * @brief main execution, handles initial input, construction of the Client, and
 * the message prompting loop
 *
